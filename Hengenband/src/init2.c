...
static void put_title(void)
{
	...
	int col;
#if H_VER_EXTRA > 0
	sprintf(title, _("変幻蛮怒 %d.%d.%d.%d(%s)", "Hengenband %d.%d.%d.%d(%s)"), H_VER_MAJOR, H_VER_MINOR, H_VER_PATCH, H_VER_EXTRA,
#else
	sprintf(title, _("変幻蛮怒 %d.%d.%d(%s)", "Hengenband %d.%d.%d(%s)"), H_VER_MAJOR, H_VER_MINOR, H_VER_PATCH,
#endif
