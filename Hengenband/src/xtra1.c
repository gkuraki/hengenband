...
void calc_bonuses(void)
{
	...
	if (buki_motteruka(INVEN_RARM) && buki_motteruka(INVEN_LARM))
	{
		...
		if ((inventory[INVEN_RARM].name1 == ART_ICINGDEATH) && (inventory[INVEN_LARM].name1 == ART_TWINCLE))
		{
			penalty1 = penalty1 / 2 - 5;
			penalty2 = penalty2 / 2 - 5;
			new_speed += 7;
	...
	if (((p_ptr->pclass == CLASS_WARRIOR) || (p_ptr->pclass == CLASS_MONK) || (p_ptr->pclass == CLASS_FORCETRAINER) || (p_ptr->pclass == CLASS_BERSERKER)) &&
		(empty_hands_status & EMPTY_HAND_RARM) && !p_ptr->hidarite)
	{
		...
		if (heavy_armor() && (p_ptr->pclass != CLASS_WARRIOR))
			p_ptr->num_blow[0] /= 2;
		else
		{
			p_ptr->to_h[0] += (p_ptr->lev / 3);
	...
	if (((p_ptr->pclass == CLASS_WARRIOR) || (p_ptr->pclass == CLASS_MONK) || (p_ptr->pclass == CLASS_FORCETRAINER) || (p_ptr->pclass == CLASS_BERSERKER)) && (empty_hands(FALSE) == (EMPTY_HAND_RARM | EMPTY_HAND_LARM))) p_ptr->ryoute = FALSE;
